...
By default the role will try the configure the NODENAME with `rabbit@{{ ansible_hostname }}` when clustering is enabled, if you want to customize the node name, you can substitute the NODENAME with the variables `rabbitmq_nodename_prefix`.
